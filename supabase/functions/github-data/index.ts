...
serve(async (req) => {
  ...
  try {
    ...
    userData.repositories?.nodes?.forEach((repo: any) => {
      repo.languages?.edges?.forEach((edge: any) => {
        ...
        if (!languageStats[langName]) {
          languageStats[langName] = { bytes: 0, color: edge.node.color || getLanguageColor(langName) };
        }
    ...
    const contributionData = userData.contributionsCollection?.contributionCalendar?.weeks?.flatMap(
    ...
    const response = {
      ...
      stats: {
        ...
        totalStars: userData.repositories?.nodes?.reduce((sum: number, repo: any) => sum + (repo.stargazerCount || 0), 0) || 0,
        totalForks: userData.repositories?.nodes?.reduce((sum: number, repo: any) => sum + (repo.forkCount || 0), 0) || 0,
      }
    };
