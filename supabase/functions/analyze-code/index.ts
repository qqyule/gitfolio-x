...
请返回以下 JSON 格式的分析结果：
{
  ...
  "personality": "基于提交时间和风格推测的开发者性格（如：夜猫子极客/严谨工程师）"
		...
		// Call Lovable AI with tool calling for structured output and retry logic
		let response
		let lastError
		const MAX_RETRIES = 3
			...
			try {
				response = await fetch(LOVABLE_AI_URL, {
					...
					body: JSON.stringify({
								...
								function: {
									...
									parameters: {
										...
										properties: {
											summary: {
												type: 'string',
												description: '3-5 sentences overall evaluation',
											},
											highlights: {
												type: 'array',
												items: { type: 'string' },
											...
											personality: {
												type: 'string',
												description: 'Inferred developer personality',
											},
										},
										required: [
											'summary',
						...
						tool_choice: {
							type: 'function',
							function: { name: 'generate_tech_profile' },
						},
					}),
				})
